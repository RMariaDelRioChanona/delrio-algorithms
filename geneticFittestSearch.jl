# genetic algorithm
function geneticFittestSearch(f, max_eval, n_wizards, bounds, mut_frac, mut_prob, duel_size; params=[false])
    """
    Genetic algorithm, selection by sorting.
    Mut frac refers to how many particles are mutated from the offspring.
    duel size is the number of particles chosen to reproduce.
    ...
    for iter = 1:max_iter
        ...
        #sort wizards by performance, keep the "n_duels" best

        cols = sort([1:n_wizards;],by= i -> f(wizards[:, i]))
        wizards = wizards[:,cols]
        score[1:n_duels] = [f(wizards[:, w]) for w = 1:n_duels]
        winners = deepcopy(wizards[:,1:n_duels])
